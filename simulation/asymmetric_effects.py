...
# simulations was used to produce Figures A-2 and A-3.
#
# -------------------------------------------------------------------------

# Alternative specification: Asymmetric treatment effects
## This differs from baseline only in how the treatment effects are created.
## Instead of symmetric treatment effects, the effect of moving from 
## group 1 to group 2 is different than the effect of moving from group 2 
## to group 3. Based on which line below is commented out, either the first
## jump will be d and the second will be 2*d, or the first will be 2*d and 
## the second will be d. This will of course make both tests more 
## powerful, so I will also change the range of d that I look at. Before,
## d was between 0 and 100, now d is between 0 and 66.
...
def master(N,m,n,cdf):
    ...
    if __name__ == "__main__":
        ...
        ############# Asymmetric treatment effects ################

	# if asymmetric down (i.e. second jump is bigger):
        d = np.expand_dims(np.transpose(np.array(
            [[0,x,3*x] for x in np.linspace(0,66,m)])),axis=(0,2)) 
	
	# if asymmetric up (i.e. first jump is bigger):
	# d = np.expand_dims(np.transpose(np.array(
	#     [[0,2*x,3*x] for x in np.linspace(0,66,m)])),axis=(0,2)) 
