...
# in Appendix A of Holt, Kwiatkowski, & Sullivan (2024). Data from these 
# simulations was used to produce Figure A-1.
#
# -------------------------------------------------------------------------

# Alternative specification: more treatment groups
## T=4 treatment groups
## n=3 observations per treatment group
## This increases power relative to the baseline specification,
## so the range of true treatment effects has been correspondingly
## reduced from [0,100] to [0,80].
...
## Right now this only works for T = 4
def findpermutations(idx):
    ...
    for i in range(int(comb(len(f),n))):
        ...
        for j in range(int(comb(len(f_),n))):
            q = next(Q)
            f__ = [x for x in f_ if x not in q]
            R = combinations(f__,n)
            for k in range(int(comb(len(f__),n))):
                r = next(R)
                s = [x for x in f__ if x not in r]
                idx_perm = np.array([p,q,r,s]).T
                yield idx_perm
...
def master(N,m,n,cdf):
    ...
    if __name__ == "__main__":
        ...
        # Modifying true treatment effects so that d goes from 0 to 80
        d = np.expand_dims(np.transpose(np.array(
            [[0,x,2*x,3*x] for x in np.linspace(0,80,m)])),axis=(0,2)) 
        global z
        z = 75 + d + e
        ...
        global num_p
        num_p = int(comb(n*T,n)*comb(n*(T-1),n)*comb(n*(T-2),n)) #Only true if T=4
        ...
        ## I think I will join the processes in groups of 12, since 369600 is divisible by 12
        ## This makes the code a little bit hard to see through
        
        pvaluesD = np.zeros((N,m))
        pvaluesJ = np.zeros((N,m))
        for i in range(int(num_p/12)): #For each group of 12 permutations (there are 30800 such groups)
            manager = mp.Manager()
            ...
            if i%10 == 0:
                print(i)
            jobs = []
            for j in range(12): #For each permutation in the group
...
for i in range(len(Fs)):
    print("cdf: ", i)
    A = master(25000,100,3,Fs[i]) #Should be N = 25000, m=100, n=3 for actual results
    print(A) #[D,J]
    BigList.append(A)
    ...
    xaxis = np.linspace(0,80,100)
    ax.plot(xaxis, A[0], label='Directional Difference')
    ax.plot(xaxis, A[1], label='Jonckheere')
